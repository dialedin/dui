...
(function ($) {
	...
	$.ui.plugin.add('resizable', 'snap', {
		start: function () {
			...
			var offsetOfParent = $this.offsetParent().offset();
			$(typeof snap == 'string' ? snap : ':data(ui-resizable)').each(function () {
				...
				var $el = $(this), offset = $el.offset(),
					p = { top: offset.top - offsetOfParent.top, left: offset.left - offsetOfParent.left },
					l = p.left + getLm($el), t = p.top + getTm($el);

				var coords = {
					l: l, t: t,
					r: l + $el.outerWidth(), b: t + $el.outerHeight() };
				inst.coords.push(coords);
			});
		},
		resize: function (event) {
			...
			var hsN = hs.length ? getN(hs) : 0,
					wsN = ws.length ? getN(ws) : 0,
					lsN = ls.length ? getN(ls) : 0,
					tsN = ts.length ? getN(ts) : 0;

			var wsT = wsN - lsN,
					hsT = hsN - tsN;

			var data = {
				width: inst.size.width + wsT,
				height: inst.size.height + hsT,
				left: inst.position.left + lsN,
				top: inst.position.top + tsN
			};

			if (inst._aspectRatio || event.shiftKey) {
				// Pick a axis that keeps its current length (the one modified the most),
				// and modify the other to keep the aspect ratio:
				if (Math.abs(wsT) > Math.abs(hsT)) {
					// Width modified more than height, get _updateRatio to provide height
					data.height = null;
				} else {
					data.width = null;
				}

				data = inst._updateRatio(data, event);
			}

			inst._updateCache(data);
		}
	});
